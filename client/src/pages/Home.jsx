...
function MembersCarousel() {
  ...
  const memberPhotos = Array.from({ length: 10 }, (_, i) => `/images/img${i + 1}.jpg`);
  const doubled = [...memberPhotos, ...memberPhotos];
  ...
  useEffect(() => {
    ...
    memberPhotos.forEach(src => {
      const img = new Image();
      img.src = src;
      img.onload = img.onerror = () => { loaded++; if (loaded === memberPhotos.length) setReady(true); };
    });
  }, []);
  ...
  return (
    <>
      <style>{`
        ...
        .video-thumb-wrap { flex-shrink: 0; width: clamp(160px, 38vw, 210px); }
        ...
        .video-thumb.active .play-icon-wrap { background: rgba(13,148,136,0.95) !important; }
        ...
        .video-title {
          margin-top: 8px; font-size: 0.78rem; font-family: sans-serif;
          color: #cbd5e1; line-height: 1.4; display: -webkit-box;
          ...
          <div className="modal-overlay" onClick={() => setShowDocsModal(false)}>
            <div className="docs-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="docs-tabs">
                {[{ id: "reg", label: "🏛️ Registration" }, { id: "tax12a", label: "📄 12A Certificate" }, { id: "tax80g", label: "📄 80G Certificate" }].map(tab => (
                  <button key={tab.id} className={`docs-tab ${activeDoc === tab.id ? "active" : ""}`} onClick={() => setActiveDoc(tab.id)}>
                    {tab.label}
                  </button>
                ))}
              </div>
              <div className="docs-body">
                <iframe key={activeDoc} src={activeDoc === "reg" ? "/images/register.pdf" : activeDoc === "tax12a" ? "/images/bipader1.pdf" : "/images/bipader2.pdf"} title="Document Viewer" />
              </div>
              <div className="docs-footer">
                <a href={activeDoc === "reg" ? "/images/register.pdf" : activeDoc === "tax12a" ? "/images/bipader1.pdf" : "/images/bipader2.pdf"} target="_blank" rel="noopener noreferrer" className="docs-open-btn">
                  Open in New Tab ↗
                </a>
              </div>
            </div>
